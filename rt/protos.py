...
for e in os.listdir(PROTO_DIR):
    if ".proto" in e:
        ...
        try:
            ...
            pass
        res = os.system(
            f"protoc --go_out=./{name} --go_opt=paths=source_relative --go-grpc_out=./{name} --go-grpc_opt=paths=source_relative -I{PROTO_DIR} {PROTO_DIR/e}"
        )
        assert res == 0, f"Failed to compile proto {e} to Go"
